...
class ProjectOperationService:
    ...
    def _apply_global_color_direct(self, color):
        try:
            ...
            for name in names:
                ...
                self._viewport.update_cloud_color(name, colors, preserve_view=True)
        except Exception:
            pass
    ...
    def denoise(self):
        ...
        try:
            ...
            if stats is not None:
                ...
                # 统一去噪后的视图复位行为，与 reset_view 一致的正交前视
                try:
                    if hasattr(self._viewport, 'reset_view'):
                        self._viewport.reset_view()
                except Exception:
                    pass
        except Exception as e:
    ...
    def _on_roi_selected(self, min_bound, max_bound, indices):
        ...
        try:
            ...
            # 距离容差（米）
            tol = float(getattr(__import__('config.settings', fromlist=['Config']).Config, 'FACADE_SIGNED_DIST_TOLERANCE', 2.5))

            # 2) 在平面约束下计算整栋建筑 BBox
            if plane is not None:
                bmin, bmax = render.compute_building_bbox_from_selection(cloud, indices, plane=plane, tol=tol)
            else:
            ...
            if bmin is None or bmax is None:
                # 回退：按选中点生成 AABB
                data = self._viewport.get_cloud_data(cloud)
                pts = np.asarray(data.get('pos'))[np.asarray(indices, dtype=int)]
    ...
    def facade_detection(self):
        ...
        # 检测前清除旧的 ROI 视觉（2D 白框与 3D BBox 一并移除）
