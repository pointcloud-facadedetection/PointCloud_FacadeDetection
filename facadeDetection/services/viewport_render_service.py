...
class ViewportRenderService:
    ...
    def highlight_facades(self, cloud_name: str, facades: list[dict], base_color=(0.75, 0.75, 0.75)):
        ...
        try:
            ...
            n = len(pos)
            if cloud_name not in self._pre_facade_colors:
                original = np.asarray(data.get('color'), dtype=np.float32)
                if original.shape == (n, 3):
                    self._pre_facade_colors[cloud_name] = original.copy()
                else:
                    self._pre_facade_colors[cloud_name] = np.tile(
                        np.asarray(base_color, dtype=np.float32).reshape(1, 3),
                        (n, 1),
                    )
