...
# 用户参数
# ============================================================
IMAGE_PATH = r"../data/southeast.jpg"          # <-- 修改为你的图片路径

OUTPUT_FULL = r"whole_building_upright_v5_full.jpg"
OUTPUT_CROP = r"whole_building_upright_v5_crop.jpg"
OUTPUT_MASK = r"whole_building_upright_v5_mask.png"
DEBUG_PATH = r"whole_building_upright_v5_debug.jpg"
...
def building_aware_crop(warped, mask, H_full, main_lines):
    """
    V5 building-aware crop：
      - x 方向仍使用 robust quantile，避免少量异常线把画面拉太宽；
      - y 顶部不再使用 1% quantile，而是使用主建筑线端点的真实最小 y；
      - 顶部使用明显更大的 asymmetric safety margin；
      - 顶部不做 inward inset；
      - 保持后续 aspect-ratio 调整只“扩张”该 base box，因此不会再次裁掉楼顶。

    这主要解决 V4 中高层建筑尖顶/屋面角点被 auto-crop 截断的问题。
    """
    Hh, Ww = mask.shape[:2]
    ...
    # --------------------------------------------------------
    # x: robust；y: 顶部必须保守，避免尖顶被 quantile 当作 outlier。
    # --------------------------------------------------------
    x_lo = float(np.quantile(pts_dst[:, 0], STRUCTURE_LOW_Q))
    x_hi = float(np.quantile(pts_dst[:, 0], STRUCTURE_HIGH_Q))

    y_top_exact = float(np.min(pts_dst[:, 1]))
    y_bottom_robust = float(np.quantile(pts_dst[:, 1], STRUCTURE_HIGH_Q))

    span_x = max(x_hi - x_lo, 1.0)
    span_y = max(y_bottom_robust - y_top_exact, 1.0)

    # --------------------------------------------------------
    # 顶部使用更大的安全 margin。
    # 楼顶往往只由一两条长竖线/斜边支持，V4 这里太激进。
    # --------------------------------------------------------
    top_margin = max(
        CROP_MARGIN_TOP_RATIO * span_y,
        TOP_SAFE_MARGIN_FULL_RATIO * Hh,
        float(TOP_SAFE_MARGIN_MIN_PX),
    )
    bottom_margin = CROP_MARGIN_BOTTOM_RATIO * span_y

    y0 = int(math.floor(y_top_exact - top_margin))
    y1 = int(math.ceil(y_bottom_robust + bottom_margin))

    y0 = max(0, min(y0, Hh - 1))
    y1 = max(y0 + 1, min(y1, Hh))

    # --------------------------------------------------------
    # 在主体 y 范围内，根据 valid mask 的左右边界压掉大黑区。
    # --------------------------------------------------------
    valid = mask > 0
    lefts, rights = [], []
    ...
    x0_struct = int(math.floor(x_lo - CROP_MARGIN_X_RATIO * span_x))
    x1_struct = int(math.ceil(x_hi + CROP_MARGIN_X_RATIO * span_x))
    ...
    # 横向/底部轻微 inward inset；顶部不 inward。
    x0 += CROP_INSET_PX
    x1 -= CROP_INSET_PX
    y0 += CROP_INSET_TOP_PX
    y1 -= CROP_INSET_BOTTOM_PX
...
if __name__ == "__main__":
    print("====================================================")
    print("Whole-building upright perspective correction V5")
    ...
    print("- V5 safe top crop: protect roof/apex vertices")
    print("====================================================")
    ...
    np.save("H_upright_v5_base.npy", H_base)
    np.save("H_upright_v5_full.npy", H_full)
    np.save("H_upright_v5_full_inv.npy", np.linalg.inv(H_full))
    np.save("H_upright_v5_crop.npy", H_crop)
    np.save("H_upright_v5_crop_inv.npy", np.linalg.inv(H_crop))
    np.save("vertical_vp_v5.npy", vp_v)
    ...
    print("\nSaved matrices:")
    print("  H_upright_v5_crop.npy      <-- original -> cropped upright")
    print("  H_upright_v5_crop_inv.npy  <-- cropped upright -> original")
    print("  H_upright_v5_full.npy")
    print("  vertical_vp_v5.npy")

    print("\nCoordinate relation:")
    print("  p_crop ~ H_upright_v5_crop @ p_original")
    print("  p_original ~ H_upright_v5_crop_inv @ p_crop")

    if SHOW_WINDOW and os.environ.get("HEADLESS", "0") != "1":
        cv2.namedWindow("Whole building upright V5 - crop", cv2.WINDOW_NORMAL)
        cv2.imshow("Whole building upright V5 - crop", upright_crop)
        cv2.waitKey(0)
