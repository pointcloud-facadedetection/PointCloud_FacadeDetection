...
#   6) 保存完整 Homography，后续可将 2D 校正图坐标映回原始图。
#
# 依赖：
...
OUTPUT_FULL = r"../data/southeast_upright_full.jpg"
OUTPUT_CROP = r"../data/southeast_upright_crop.jpg"
OUTPUT_MASK = r"../data/southeast_upright_mask.png"
DEBUG_PATH = r"../data/southeast_upright_debug.jpg"
...
if __name__ == "__main__":
    print("====================================================")
    print("Whole-building upright perspective correction V3")
    ...
    # Step 6: building-aware crop
    if AUTO_CROP:
        upright_crop, crop_mask, H_crop, crop_box = building_aware_crop(
            upright_full, valid_mask, H_full, main_lines
        )
    else:
        upright_crop = upright_full
        crop_mask = valid_mask
        H_crop = H_full
        crop_box = (0, 0, upright_full.shape[1], upright_full.shape[0])

    print("Crop box [x0,y0,x1,y1]:", crop_box)
    ...
    # Step 9: save matrices
    np.save("H_upright_v3_base.npy", H_base)
    np.save("H_upright_v3_full.npy", H_full)
    np.save("H_upright_v3_full_inv.npy", np.linalg.inv(H_full))
    np.save("H_upright_v3_crop.npy", H_crop)
    np.save("H_upright_v3_crop_inv.npy", np.linalg.inv(H_crop))
    np.save("vertical_vp_v3.npy", vp_v)
    ...
    print("\nSaved matrices:")
    print("  H_upright_v3_crop.npy      <-- original -> cropped upright")
    print("  H_upright_v3_crop_inv.npy  <-- cropped upright -> original")
    print("  H_upright_v3_full.npy")
    print("  vertical_vp_v3.npy")

    print("\nCoordinate relation:")
    print("  p_crop ~ H_upright_v3_crop @ p_original")
    print("  p_original ~ H_upright_v3_crop_inv @ p_crop")

    cv2.namedWindow("Whole building upright V3 - crop", cv2.WINDOW_NORMAL)
    cv2.imshow("Whole building upright V3 - crop", upright_crop)
    cv2.waitKey(0)
