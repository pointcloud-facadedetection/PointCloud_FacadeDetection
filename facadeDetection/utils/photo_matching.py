...
class PhotoMatchResult:
    ...
    homography_uv_to_photo: np.ndarray | None = None
    homography_inlier_count: int = 0
    homography_rmse_px: float = 0.0
...
@dataclass(frozen=True)
class PlaneFrame:
    """热力图与照片共享的立面局部坐标系。v 轴方向由 raster 约定决定。"""
    origin: np.ndarray
    u_axis: np.ndarray
    v_axis: np.ndarray
    plane_model: np.ndarray | None = None
...
def world_to_plane_uv(points, plane_frame):
    """将世界坐标转换为质量热力图使用的局部 (u,v)，不重新推导轴向。"""
    frame = _as_plane_frame(plane_frame)
    xyz = np.asarray(points, dtype=np.float64).reshape(-1, 3)
    d = xyz - frame.origin
    return np.column_stack((d @ frame.u_axis, d @ frame.v_axis))


def plane_uv_to_photo(uv, uv_to_photo):
    uv = np.asarray(uv, dtype=np.float64).reshape(-1, 2)
    h = np.hstack((uv, np.ones((len(uv), 1)))) @ np.asarray(uv_to_photo, dtype=np.float64).reshape(3, 3).T
    if np.any(np.abs(h[:, 2]) < 1e-10):
        raise ValueError('平面单应变换齐次分母无效')
    return h[:, :2] / h[:, 2:3]


def heatmap_pixel_to_photo(shape, uv_bounds, uv_to_photo):
    """返回热力图每个像素中心对应的照片像素坐标 (H,W,2)。"""
    height, width = map(int, shape[:2])
    u0, v0, u1, v1 = map(float, uv_bounds)
    xs = u0 + (np.arange(width, dtype=np.float64) + .5) * (u1-u0) / width
    ys = v1 - (np.arange(height, dtype=np.float64) + .5) * (v1-v0) / height
    uu, vv = np.meshgrid(xs, ys)
    return plane_uv_to_photo(np.column_stack((uu.ravel(), vv.ravel())), uv_to_photo).reshape(height, width, 2)


def warp_heatmap_rgba_to_photo(heatmap_rgba, valid_mask, uv_bounds, uv_to_photo, photo_size):
    """通过 pixel -> UV -> photo 的单应映射将透明热力图投到原始照片。"""
    rgba = np.asarray(heatmap_rgba, dtype=np.uint8)
    if rgba.ndim != 3 or rgba.shape[2] != 4:
        raise ValueError('热力图必须为 RGBA')
    h, w = rgba.shape[:2]
    corners_uv = np.array(((uv_bounds[0], uv_bounds[1]), (uv_bounds[2], uv_bounds[1]),
                           (uv_bounds[2], uv_bounds[3]), (uv_bounds[0], uv_bounds[3])), dtype=np.float64)
    src = np.array(((0, h-1), (w-1, h-1), (w-1, 0), (0, 0)), dtype=np.float32)
    dst = plane_uv_to_photo(corners_uv, uv_to_photo).astype(np.float32)
    hmap = cv2.getPerspectiveTransform(src, dst)
    pw, ph = map(int, photo_size)
    mask = np.asarray(valid_mask, dtype=np.uint8) if valid_mask is not None else rgba[:, :, 3]
    out = cv2.warpPerspective(rgba, hmap, (pw, ph), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT)
    out[:, :, 3] = cv2.warpPerspective(mask, hmap, (pw, ph), flags=cv2.INTER_NEAREST)
    return out


def _as_plane_frame(value):
    if isinstance(value, PlaneFrame):
        return value
    if value is None or len(value) < 3:
        raise ValueError('plane_frame 必须包含 origin/u_axis/v_axis')
    origin, u, v = (np.asarray(value[i], dtype=np.float64).reshape(3) for i in range(3))
    if not all(np.isfinite(x).all() for x in (origin, u, v)):
        raise ValueError('plane_frame 包含无效坐标')
    u /= max(np.linalg.norm(u), 1e-12); v /= max(np.linalg.norm(v), 1e-12)
    plane = None if len(value) < 4 or value[3] is None else np.asarray(value[3], dtype=np.float64).reshape(4)
    return PlaneFrame(origin, u, v, plane)


def _homography_from_pose(K, R, t, frame):
    f = _as_plane_frame(frame)
    H = np.asarray(K) @ np.column_stack((np.asarray(R) @ f.u_axis, np.asarray(R) @ f.v_axis, np.asarray(R) @ f.origin + np.asarray(t)))
    return H / H[2, 2]


def _homography_from_pose_with_axes(K, R, t, origin, u_axis, v_axis):
    """从相机位姿直接解析计算立面平面到照片像素的单应。"""
    ...
    H = K @ np.column_stack([R @ u_axis, R @ v_axis, R @ origin + t])
    if abs(H[2, 2]) > 1e-12:
        H = H / H[2, 2]
    return H


def _homography_consistent(H1, H2, correspondences, origin, u_axis, v_axis,
                            tol_px=20.0):
    """检查两个 H 在 correspondence 上的中位差异是否小于阈值。"""
    if not correspondences:
        return False
    src_uv = []
    for c in correspondences:
        op = c.get('object_point')
        if op is None:
            continue
        op = np.asarray(op, dtype=np.float64).reshape(-1)
        if op.size != 3 or not np.isfinite(op).all():
            continue
        d = op - origin
        src_uv.append([float(d @ u_axis), float(d @ v_axis)])
    if len(src_uv) < 4:
        return False
    src_uv = np.asarray(src_uv, dtype=np.float32).reshape(1, -1, 2)
    proj1 = cv2.perspectiveTransform(src_uv, np.asarray(H1, np.float64)).reshape(-1, 2)
    proj2 = cv2.perspectiveTransform(src_uv, np.asarray(H2, np.float64)).reshape(-1, 2)
    median_diff = float(np.median(np.linalg.norm(proj1 - proj2, axis=1)))
    print(f'[PhotoMatch] homography_consistency: median_diff={median_diff:.2f}px',
          flush=True)
    return median_diff < tol_px


def _solve_plane_homography_robust(correspondences, plane_frame,
                                    camera_matrix=None,
                                    rotation=None,
                                    translation=None):
    """求解立面平面到照片像素的单应 H_uv_to_photo。

    策略（按优先级）：
      1) 从 K/R/t 解析计算 H_pose —— 无对应点依赖，稳健
      2) 过滤 correspondence（只保留离平面 < 0.5m 的点）后 RANSAC 求 H_ransac
      3) 若两者都存在，用一致性检查决定；不一致时**必须**用 H_pose
      4) 只要求 inlier >= 6（4 点是过拟合临界，必须提高）
    ...
    """
    # ---- 解析平面参考系 ----
    if plane_frame is None or len(plane_frame) < 3:
        return None, {}
    origin = np.asarray(plane_frame[0], dtype=np.float64).reshape(3)
    u_axis = np.asarray(plane_frame[1], dtype=np.float64).reshape(3)
    v_axis = np.asarray(plane_frame[2], dtype=np.float64).reshape(3)
    if not (np.isfinite(origin).all() and np.isfinite(u_axis).all()
            and np.isfinite(v_axis).all()):
        return None, {}
    u_axis = u_axis / max(np.linalg.norm(u_axis), 1e-12)
    v_axis = v_axis / max(np.linalg.norm(v_axis), 1e-12)
    plane_model = (
        np.asarray(plane_frame[3], dtype=np.float64).reshape(4)
        if len(plane_frame) >= 4 and plane_frame[3] is not None else None
    )
    ...
    if (camera_matrix is not None
        ...
        try:
            H_pose = _homography_from_pose_with_axes(
                camera_matrix, rotation, translation,
                origin, u_axis, v_axis)
            print('[PhotoMatch] homography_from_pose ready', flush=True)
        except Exception as exc:
            print(f'[PhotoMatch] homography_from_pose failed: {exc}', flush=True)
            H_pose = None

    # ---- 2. 过滤 correspondence 后 RANSAC ----
    H_ransac = None
    H_ransac_meta = {}
    if correspondences and len(correspondences) >= 6:
        src_uv, dst_xy = [], []
        dropped_far = 0
        for c in correspondences:
            op = c.get('object_point')
            ip = c.get('image_point')
            if op is None or ip is None:
                continue
            op = np.asarray(op, dtype=np.float64).reshape(-1)
            ip = np.asarray(ip, dtype=np.float64).reshape(-1)
            if op.size != 3 or ip.size != 2:
                continue
            if not (np.isfinite(op).all() and np.isfinite(ip).all()):
                continue
            # 距离平面过滤：只保留贴近平面的点
            if plane_model is not None:
                n = float(np.linalg.norm(plane_model[:3]))
                if n > 1e-12:
                    dist = abs(float(op @ plane_model[:3] + plane_model[3])) / n
                    if dist > 10.0:
                        dropped_far += 1
                        continue
            d = op - origin
            src_uv.append([float(d @ u_axis), float(d @ v_axis)])
            dst_xy.append([float(ip[0]), float(ip[1])])
        print(f'[PhotoMatch] correspondence filter: kept={len(src_uv)} '
              f'dropped_far={dropped_far}', flush=True)

        if len(src_uv) >= 6:
            src_uv = np.asarray(src_uv, dtype=np.float32)
            dst_xy = np.asarray(dst_xy, dtype=np.float32)
            span_u = float(src_uv[:, 0].max() - src_uv[:, 0].min())
            span_v = float(src_uv[:, 1].max() - src_uv[:, 1].min())
            if span_u < 0.5 or span_v < 0.5:
                print(f'[PhotoMatch] homography(RANSAC) rejected: '
                      f'span too small span_u={span_u:.2f}m span_v={span_v:.2f}m',
                      flush=True)
            else:
                H, mask = cv2.findHomography(
                    src_uv, dst_xy, cv2.RANSAC, 5.0,
                    maxIters=5000, confidence=0.995)
                if H is not None and mask is not None:
                    inliers = int(mask.sum())
                    # 最低 inlier 从 4 提到 6，杜绝 4 点过拟合
                    if inliers >= 6:
                        inlier_mask = mask.ravel().astype(bool)
                        proj = cv2.perspectiveTransform(src_uv[None], H)[0]
                        err = np.linalg.norm(proj - dst_xy, axis=1)
                        rmse = float(np.sqrt(np.mean(err[inlier_mask] ** 2)))
                        H_ransac = H
                        H_ransac_meta = {
                            'homography_inlier_count': inliers,
                            'homography_rmse_px': rmse,
                        }
                        print(f'[PhotoMatch] homography_uv_to_photo(RANSAC) ok: '
                              f'inliers={inliers}/{len(src_uv)} '
                              f'rmse={rmse:.2f}px '
                              f'span_u={span_u:.2f}m span_v={span_v:.2f}m',
                              flush=True)
                    else:
                        print(f'[PhotoMatch] homography(RANSAC) rejected: '
                              f'inliers={inliers}<6 (over-fitting risk)',
                              flush=True)

    # ---- 3. 三路决策 ----
    if H_pose is not None and H_ransac is not None:
        if _homography_consistent(H_pose, H_ransac, correspondences,
                                   origin, u_axis, v_axis, tol_px=20.0):
            print('[PhotoMatch] homography: pose & ransac consistent → '
                  'prefer RANSAC', flush=True)
            H = H_ransac
            meta = H_ransac_meta
        else:
            print('[PhotoMatch] homography: pose & ransac INCONSISTENT → '
                  'use pose (RANSAC over-fitted)', flush=True)
            H = H_pose
            meta = {'homography_inlier_count': 0,
                    'homography_rmse_px': 0.0}
    elif H_pose is not None:
        print('[PhotoMatch] homography: use pose only', flush=True)
        H = H_pose
        meta = {'homography_inlier_count': 0,
                'homography_rmse_px': 0.0}
    elif H_ransac is not None:
        print('[PhotoMatch] homography: use RANSAC only', flush=True)
        H = H_ransac
        meta = H_ransac_meta
    else:
        print('[PhotoMatch] homography: unavailable', flush=True)
        return None, {}
...
def estimate_photo_cloud_match(
    ...
    *,
    ...
    # 若提供了与热力图同源的平面参考系，则导出 uv→photo 单应
    H = None
    h_meta = {}
    if plane_frame is not None and matched.get('pose_estimated'):
        H, h_meta = _solve_plane_homography_robust(
            correspondences=matched.get('correspondences') or [],
            plane_frame=plane_frame,
            camera_matrix=np.asarray(matched['camera_matrix'], dtype=np.float64).reshape(3, 3),
            rotation=np.asarray(matched['rotation_matrix'], dtype=np.float64).reshape(3, 3),
            translation=np.asarray(matched['translation_vector'], dtype=np.float64).reshape(3),
        )
    ...
    return PhotoMatchResult(
        ...
        homography_uv_to_photo=H,
        homography_inlier_count=int(h_meta.get('homography_inlier_count', 0)),
        homography_rmse_px=float(h_meta.get('homography_rmse_px', 0.0)),
        plane_origin=h_meta.get('plane_origin'),
        plane_u_axis=h_meta.get('plane_u_axis'),
        plane_v_axis=h_meta.get('plane_v_axis'),
        plane_model=h_meta.get('plane_model'),
    )
...
def _overlay_points_on_rectified(
    ...
    *,
    ...
    from utils.heatmap_colors import (
        GRAY_BGR,
...
__all__ = [
    ...
    'world_to_plane_uv',
    'plane_uv_to_photo',
    'heatmap_pixel_to_photo',
    'warp_heatmap_rgba_to_photo',
]
